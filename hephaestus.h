...
#define h_make_version(major, minor, patch)                                    \
...
#define h_make_info_app(in_app_name, in_app_version, in_engine_name,           \
                        in_engine_version, in_api_version)                     \
  (h_info_app) {                                                               \
...
#define h_make_info_instance(in_app_info, in_layer_count, in_layer_names,      \
                             in_extension_count, in_extension_names)           \
  (h_info_instance) {                                                          \
...
#define h_make_info_device(in_queue_create_info_count, in_queue_create_infos,  \
                           in_enabled_layer_count, in_enabled_layer_names,     \
                           in_enabled_extension_count,                         \
                           in_enabled_extension_names, in_enabled_features)    \
  (h_info_device) {                                                            \
...
#define h_make_info_device_queue(in_queue_family_index, in_queue_count,        \
                                 in_queue_priorities)                          \
  (h_info_device_queue) {                                                      \
...
h_device h_new_device(h_physical_device in_physical_device,
                      h_info_device in_info) {
...
h_queue h_new_queue(h_device in_device, uint32_t family_index,
...
#define h_make_info_image(                                                     \
    in_image_type, in_extent, in_mip_levels, in_array_layers, in_format,       \
    in_tiling, in_initial_layout, in_usage, in_sharing_mode, in_samples)       \
  (h_info_image) {                                                             \
    .sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, .imageType = in_image_type,  \
    .extent = in_extent, .mipLevels = in_mip_levels,                           \
    .arrayLayers = in_array_layers, .format = in_format, .tiling = in_tiling,  \
    .initialLayout = in_initial_layout, .usage = in_usage,                     \
    .sharingMode = in_sharing_mode, .samples = in_samples,                     \
  }

typedef VkImageView h_image_view;
typedef VkImageViewCreateInfo h_info_image_view;
#define h_make_info_image_view(in_image, in_view_type, in_format,              \
                               in_components, in_subresource_range)            \
  (h_info_image_view) {                                                        \
    .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO, .image = in_image,      \
    .viewType = in_view_type, .format = in_format,                             \
    .components = in_components, .subresourceRange = in_subresource_range,     \
  }
...
#define h_make_info_shader_module(in_code, in_code_size)                       \
  (h_info_shader_module) {                                                     \
    .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, .pCode = in_code,    \
    .codeSize = in_code_size,                                                  \
  }

h_shader_module h_new_shader_module(h_device in_device,
                                    h_info_shader_module in_info) {
...
#define h_make_info_pipeline_shader_stage(in_stage, in_module, in_name)        \
  (h_info_pipeline_shader_stage) {                                             \
    .sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,              \
    .stage = in_stage, .module = in_module, .pName = in_name,                  \
  }

//

typedef VkVertexInputBindingDescription h_vertex_binding;
#define h_make_vertex_binding_per_vertex(in_binding, in_form_size)             \
  (h_vertex_binding) {                                                         \
    .binding = in_binding, .stride = in_form_size,                             \
    .inputRate = VK_VERTEX_INPUT_RATE_VERTEX                                   \
  }
#define h_make_vertex_binding_per_instance(in_binding, in_form_size)           \
  (h_vertex_binding) {                                                         \
    .binding = in_binding, .stride = in_form_size,                             \
...
#define h_make_info_pipeline_vertex(in_binding_count, in_binding,              \
                                    in_attribute_count, in_attributes)         \
...
#define h_make_info_pipeline_assembly(in_topology)                             \
...
#define h_make_viewport(in_x, in_y, in_width, in_height, in_depth_min,         \
                        in_depth_max)                                          \
  (h_viewport) {                                                               \
    .x = in_x, .y = in_y, .width = in_width, .height = in_height,              \
    .minDepth = in_depth_min, .maxDepth = in_depth_max,                        \
  }

typedef VkRect2D h_scissor;

typedef VkPipelineViewportStateCreateInfo h_info_pipeline_viewport;
#define h_make_info_pipeline_viewport(in_viewport_count, in_viewports,         \
                                      in_scissor_count, in_scissors)           \
  (h_info_pipeline_viewport) {                                                 \
    .sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO,            \
    .viewportCount = in_viewport_count, .pViewports = in_viewports,            \
    .scissorCount = in_scissor_count, .pScissors = in_scissors,                \
  }

//

typedef VkPipelineRasterizationStateCreateInfo h_info_pipeline_raster;
#define h_make_info_pipeline_raster(in_polygon_mode, in_line_width,            \
                                    in_cull_mode, in_front_face)               \
  (h_info_pipeline_raster) {                                                   \
    .sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO,       \
    .polygonMode = in_polygon_mode, .lineWidth = in_line_width,                \
    .cullMode = in_cull_mode, .frontFace = in_front_face,                      \
  }
...
#define h_make_info_render_pass(in_attachment_count, in_attachments,           \
                                in_subpass_count, in_subpasses,                \
                                in_dependency_count, in_dependencies)          \
  (h_info_render_pass) {                                                       \
    .sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO,                        \
    .attachmentCount = in_attachment_count, .pAttachments = in_attachments,    \
    .subpassCount = in_subpass_count, .pSubpasses = in_subpasses,              \
    .dependencyCount = in_dependency_count, .pDependencies = in_dependencies,  \
  }

h_render_pass h_new_render_pass(h_device in_device,
                                h_info_render_pass in_info) {
...
#define h_make_info_semaphore()                                                \
  (h_info_semaphore) { .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO, }
...
#define h_make_info_pipeline(                                                  \
    in_stage_count, in_stages, in_vertex_input_state, in_input_assembly_state, \
    in_tessellation_state, in_viewport_state, in_rasterization_state,          \
    in_multisample_state, in_depth_stencil_state, in_color_blend_state,        \
    in_dynamic_state, in_layout, in_render_pass, in_subpass,                   \
    in_base_pipeline_handle, in_base_pipeline_index)                           \
  (h_info_pipeline){                                                           \
...
h_pipeline h_new_pipeline(h_device in_device, h_info_pipeline in_info) {
  ...
  vkCreateGraphicsPipelines(in_device, VK_NULL_HANDLE, 1, &in_info, NULL,
                            &temp_pipeline);
...
#define h_make_info_pipeline_layout(in_set_layout_count, in_set_layouts,       \
...
h_pipeline_layout h_new_pipeline_layout(h_device in_device,
                                        h_info_pipeline_layout in_info) {
...
#define h_make_info_descriptor_set(in_descriptor_pool,                         \
                                   in_descriptor_set_count, in_set_layouts)    \
...
h_descriptor_set h_new_descriptor_set(h_device in_device,
                                      h_info_descriptor_set in_info) {
...
#define h_make_info_framebuffer(in_render_pass, in_attachment_count,           \
                                in_attachments, in_width, in_height,           \
                                in_layers)                                     \
  (h_info_framebuffer) {                                                       \
    .sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,                        \
    .renderPass = in_render_pass, .attachmentCount = in_attachment_count,      \
    .pAttachments = in_attachments, .width = in_width, .height = in_height,    \
    .layers = in_layers,                                                       \
  }

h_framebuffer h_new_framebuffer(h_device in_device,
                                h_info_framebuffer in_info) {
...
#define h_make_info_command_pool(in_queue_family_index)                        \
  (h_info_command_pool) {                                                      \
    .sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,                       \
    .queueFamilyIndex = in_queue_family_index,                                 \
  }

h_command_pool h_new_command_pool(h_device in_device,
                                  h_info_command_pool in_info) {
...
#define h_make_info_command_buffer(in_command_pool, in_level)                  \
  (h_info_command_buffer) {                                                    \
    .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,                   \
    .commandPool = in_command_pool, .level = in_level,                         \
    .commandBufferCount = 1,                                                   \
  }

h_command_buffer h_new_command_buffer(h_device in_device,
                                      h_info_command_buffer in_info) {
  h_command_buffer *temp_buffer = malloc(sizeof(h_command_buffer));
  vkAllocateCommandBuffers(in_device, &in_info, temp_buffer);
  return *temp_buffer;
}

void allocate_h_command_buffers(h_device in_device,
                                h_info_command_buffer in_info,
                                h_command_buffer *command_buffers) {
...
#define h_make_info_submit(in_ready_semaphore_count, in_ready_semaphores,      \
                           in_wait_stages, in_command_count, in_commands,      \
                           in_done_semaphore_count, in_done_semaphores)        \
  (h_info_submit) {                                                            \
...
void h_submit_queue(h_queue in_queue, h_info_submit in_submit,
                    h_fence in_fence) {
